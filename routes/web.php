...
// ============ ADMIN ROUTES ============
Route::middleware(['auth', 'role:admin,admin_support,admin_stnk'])
    ->prefix('admin')
    ->name('admin.')
    ->group(function () {
        ...
        // Customer CRUD - Shared
        Route::resource('/customers', \App\Http\Controllers\CustomerController::class);

        // Member Card generation - admin & admin_support
        Route::post('/customers/{customer}/member-card/generate', [\App\Http\Controllers\MemberCardController::class, 'generate'])->name('member_cards.generate')->middleware('role:admin,admin_support');
        Route::get('/customers/{customer}/member-card/preview', [\App\Http\Controllers\MemberCardController::class, 'preview'])->name('member_cards.preview')->middleware('role:admin,admin_support');
        Route::get('/customers/{customer}/member-card/print', [\App\Http\Controllers\MemberCardController::class, 'print'])->name('member_cards.print')->middleware('role:admin,admin_support');
        Route::post('/customers/{customer}/member-card/regenerate', [\App\Http\Controllers\MemberCardController::class, 'regenerate'])->name('member_cards.regenerate')->middleware('role:admin,admin_support');

        // Vehicle CRUD - Shared
        Route::resource('/vehicles', \App\Http\Controllers\VehicleController::class);
        // Vehicle STNK Status Update - admin & admin_stnk
        Route::post('/vehicles/{vehicle}/update-stnk', [\App\Http\Controllers\VehicleController::class, 'updateStnkStatus'])->name('vehicles.update_stnk')->middleware('role:admin,admin_stnk');
        ...
        // Scan - admin & admin_support
        Route::get('/scan', [\App\Http\Controllers\ScanController::class, 'index'])->name('scan.index')->middleware('role:admin,admin_support');
        Route::post('/scan/validate', [\App\Http\Controllers\ScanController::class, 'validateToken'])->name('scan.validate')->middleware('role:admin,admin_support');

        // Service Schedules - admin & admin_support
        Route::get('/service-schedules', [\App\Http\Controllers\ServiceScheduleController::class, 'index'])->name('service_schedules.index')->middleware('role:admin,admin_support');
        Route::patch('/service-schedules/{schedule}/complete', [\App\Http\Controllers\ServiceScheduleController::class, 'complete'])->name('service_schedules.complete')->middleware('role:admin,admin_support');
        Route::patch('/service-schedules/{schedule}/cancel', [\App\Http\Controllers\ServiceScheduleController::class, 'cancel'])->name('service_schedules.cancel')->middleware('role:admin,admin_support');

        // S2: WhatsApp Reminder - admin & admin_support
        Route::get('/whatsapp-reminders', [\App\Http\Controllers\WhatsAppReminderController::class, 'index'])->name('whatsapp.index')->middleware('role:admin,admin_support');
        Route::post('/whatsapp-reminders/send', [\App\Http\Controllers\WhatsAppReminderController::class, 'send'])->name('whatsapp.send')->middleware('role:admin,admin_support');
        Route::post('/whatsapp-reminders/{notification}/retry', [\App\Http\Controllers\WhatsAppReminderController::class, 'retry'])->name('whatsapp.retry')->middleware('role:admin,admin_support');
